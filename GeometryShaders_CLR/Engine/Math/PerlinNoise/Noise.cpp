#include "Noise.hpp"
//Credited to Brian "Squirrel" Eiserloh
...
float ComputePerlinNoiseValueAtPosition2DWithSeed( const Vector2& position, int seed, float perlinNoiseGridCellSize,
	int numOctaves, float baseAmplitude, float persistance )
{
	int numOctavesRemaining = numOctaves;
	float amplitude = baseAmplitude;
	...
	while( numOctavesRemaining > 0 )
	{
		...
		Vector2 southwestNoiseGradient = GetPseudoRandomNoiseUnitVector2DWithSeed( perlinCell.x, perlinCell.y, seed );
		Vector2 southeastNoiseGradient = GetPseudoRandomNoiseUnitVector2DWithSeed( perlinCell.x + 1, perlinCell.y, seed );
		Vector2 northeastNoiseGradient = GetPseudoRandomNoiseUnitVector2DWithSeed( perlinCell.x + 1, perlinCell.y + 1, seed );
		Vector2 northwestNoiseGradient = GetPseudoRandomNoiseUnitVector2DWithSeed( perlinCell.x, perlinCell.y + 1, seed );
